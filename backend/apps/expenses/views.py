...
from .permissions import IsExpenseMember
...
class ExpenseCategoryViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [permissions.IsAuthenticated, IsExpenseMember]
    search_fields = ['name']

    def get_queryset(self):
        business = getattr(self.request, 'business', None)
        if self.request.user.is_platform_admin and not business:
            return ExpenseCategory.objects.filter(is_deleted=False)
        if business:
            return ExpenseCategory.objects.filter(business=business, is_deleted=False)
...
class ExpenseViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [permissions.IsAuthenticated, IsExpenseMember]
    filterset_fields = ['category', 'branch', 'date']
    ...
    def get_queryset(self):
        business = getattr(self.request, 'business', None)
        if self.request.user.is_platform_admin and not business:
            return Expense.objects.filter(is_deleted=False).select_related('category', 'created_by', 'branch')
        if business:
            return Expense.objects.filter(
                business=business, is_deleted=False
            ).select_related('category', 'created_by', 'branch')
        return Expense.objects.none()

    def perform_create(self, serializer):
        business = getattr(self.request, 'business', None)
        if business:
            instance = serializer.save(business=business, created_by=self.request.user)
        else:
